...
\begin{table}[h]
    ...
    \begin{NiceTabular}{c c c}[hvlines,color-inside]
        ...
        \Block{5-1}{\textit{Model \& Epoch-wise} \\ \textit{double-descent}} & \Block{5-1}{Exploraremos en profundidad el \textit{Deep Double Descent} \\ asociado a la complejidad del modelo y al número de épocas, \\ utilizando los diversos conjuntos de datos y arquitecturas.} & \Block{5-1}{\ref{subsec:model-epoch-wise}} \\ \\ \\ \\ \\
...
\begin{table}[h]
    ...
    \renewcommand{\arraystretch}{0.9} 
    \begin{NiceTabular}{c c c}[hvlines,color-inside]
        \Block{2-1}{\textbf{N° total experimentos}} & \Block{2-1}{\textbf{N° horas GPU (servidor UGR)}} & \Block{2-1}{\textbf{Equivalencia días}} \\ \\
        
        \Block{2-1}{$40$} & \Block{2-1}{$\approx 1795$h} & \Block{2-1}{$\approx 75$d\footnotemark} \\ \\ 
...
\end{table}
\footnotetext{Este tiempo total debe ser dividido entre $3$, pues se utilizaron tres nodos de cómputo en paralelo. Por tanto, el tiempo real de ejecución sobre las GPUs de la UGR fue de aproximadamente $25$ días.}
...
\end{table}
\footnotetext{Los polinomios de Legendre forman un sistema de polinomios completos y ortogonales en el intervalo $[-1, 1]$.}
