...
Este comportamiento, en el que la curva del error de generalización exhibe dos descensos, puede apreciarse en la Figura~\ref{fig:planteamiento-teorico-dd.png} y lo denotaremos como \textbf{doble descenso profundo}, o simplemente \textbf{doble descenso}. En este contexto, el adjetivo profundo hace referencia al uso de redes neuronales profundas como modelo base.
...
Estas propiedades incluyen funciones de pérdida comunes para problemas de clasificación binaria, incluyendo la función logística y la exponencial. Además, estas propiedades implican que $\mathcal{L}(w)$ (véase ecuación~\eqref{eq:funcion-perdida}) es una función $\beta \sigma^{2}_{\max}(X)$-suave (ligado al hecho de que $\ell$ es $\beta$-suave), donde $\sigma_{\max}(X)$ es el máximo valor singular de la matriz $X \in \mathbb{R}^{D \times N}$ que contiene los datos de entrenamiento.
...
\begin{table}[h]
    ...
    \end{NiceTabular}
    \caption[Resumen comparativo de las diferencias entre el paradigma clásico y el paradigma moderno.]{Resumen comparativo de las diferencias entre el paradigma clásico y el paradigma moderno.}\label{tabla:tabla-conclusion}
\end{table}
